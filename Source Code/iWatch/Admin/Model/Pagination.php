...
class Pagination
{
    ...
    private function getPrePage()
    {
        ...
        if ($this->getCurrentPage() === 1) {
            ...
            return '<li class="page-item"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() - 1) . '" >Pre</a></li>';
        }
    }
    ...
    private function getNextPage()
    {
        ...
        if ($this->getCurrentPage() >= $this->gettotalPage()) {
            ...
            // còn không thì trả về HTML code
            return '<li class="page-item"><a class="page-link" href="' . $_SERVER['PHP_SELF'] . '?' . $this->config['querystring'] . '=' . ($this->getCurrentPage() + 1) . '" >Next</a></li>';
        }
    }
    ...
    public function getPagination()
    {
        ...
        if (isset($this->config['full']) && $this->config['full'] === false) {
            // nếu không thì
            $data .= ($this->getCurrentPage() - 3) > 1 ? '<li class="page-item">...</li>' : '';
